...
Dieser Leitfaden beschreibt die technische Basis zur Anbindung von Primärsystemen an KIM.
[.green-background]
****
*Abgrenzung* +
...
****
*Empfehlung* +
Es wird empfohlen, diesen Leitfaden in Verbindung mit der link:https://gemspec.gematik.de/docs/gemSpec/gemSpec_CM_KOMLE/latest/[Spezifikation des KIM-Clientmoduls] zu betrachten, da die Anforderungen an die Schnittstelle und damit Integration in Primärsystem darin definiert sind.
****

== Übersicht
Das sichere Übermittlungsverfahren KIM stellt Primärsystemen die Möglichkeit zur Verfügung, mit anderen KIM-Teilnehmern (Ärzten, Arztpraxen, Krankenhäusern usw.) eine Ende-zu-Ende-gesicherte E-Mail-Kommunikation zu führen. Die *Ver-/Entschlüsselung sowie die Sicherstellung der Integrität von KIM-Nachrichten* (E-Mails) wird mittels und damit in Abhängigkeit zu Funktionen der dezentralen TI-Komponenten: TI-Konnektor, TI-Kartenterminals & Smartcards (SM-B und HBA) *durch das KIM-Clientmodul umgesetzt*. 

== Abkürzungen und Begeriffe
Generell sind Abkürzungen und Begriffe im link:https://gemspec.gematik.de/docs/gemSpec/gemSpec_KOMLE/latest/[gematik Glossar] beschrieben. Die folgenden Abkürzungen und Begriffe sind für dieses Dokument wesentlich:
...
|Software, welche als Client Funktionen der TI-Plattform (KIM-Clientmodul, TI-Konnektor) nutzt
|===
...
|2
|Registrierung bei einem KIM-Provider und erhalt einer KIM-Adresse
|Nutzer, LE(I) oder Organisation

|3
|Die KIM-Fachdaten (u.a. KIM-Adresse, entspricht einer E-Mail-Adresse) sind im VZD der TI-Identität des Nutzers angegeben
|KIM-Provider, automatisch bei Registrierung KIM-Adresse durch Nutzer

|4
|Der Nutzer verfügt über eine freigeschaltete SM-B (und ggf. einen freigeschalteten HBA)
|Nutzer, LE(I) oder Organisation

|5
|Funktionsfähige, online TI-Anbindung des Primärsystems (TI-Konnektor, VPN, ...) ist vorhanden
|Nutzer, LE(I) oder Organisation, Dienstleister

|6
|Funktionsfähige SMTP- & POP3 E-Mail-Client-Funktion im Primärsystem oder standalone E-Mail-Clientsoftware, sowie Konfiguration
