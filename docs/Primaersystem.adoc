...
KIM-Nachrichten können nur versendet werden, wenn die KIM-Adressen der Empfänger im Verzeichnisdienst existieren. Daher ist es erforderlich, dass das Primärsystem die lokal gespeicherten KIM-Adressen regelmäßig mit den Daten im VZD vergleicht und nicht mehr im VZD vorhandene KIM-Adressen aus dem lokalen Speicher entfernt und gegebenenfalls durch aktuelle KIM-Adressen aus dem VZD ersetzt.
Es wird empfohlen die lokal gespeicherten KIM-Adressen einmal täglich zu aktualisieren.
...
==== Umgang mit Fehlernachrichten

Im KIM Protokoll sind Fehlernachrichten vorgesehen, die dazu dienen den Nutzer über Probleme beim Versand oder Empfang von KIM-Nachrichten zu informieren. Das Primärsystem kann den Nutzer bei der Auswertung von Fehlernachrichten unterstützen. Jede Fehlernachricvht enthält einen Fehlercode im Headerelement ´X-KIM-Fehlermeldung´. In *[gemSpec_CM_KOMLE] Tab_Fehlercodes_KOMLE-Clientmodule* sind die Fehlercodes definiert.
Es wird empfohlen, den Nutzer im Primärsystem auf Fehlermeldungen hinzuweisen und geeignete Maßnahmen zum Umgang mit der Fehlermeldung vorzuschlagen um Folgefehler zu vermeiden.
