...
Einige KIM Anbieter unterstützen die Einbindung eines zentralen AV-Service direkt in ihrem KIM-Clientmodul. Für eine solche Lösung sind keine zusätzlichen Änderungen am Primärsystem erforderlich. Bitte erfragen Sie bei Ihrem KIM-Anbieter, ob und in welcher Weise eine solche Lösung unterstützt wird.

=== Mailserver und AV-Scanner mit SMTP-KIM-Adapter
In dieser Lösung wird ein eigener, in der Regel schon vorhandener, Mailserver mit angeschlossenem AV-Scanner mit Hilfe eines SMTP-KIM-Adapters an das KIM-Clientmodul angebunden. Bei der Lösung wird der AV-Service über den Mailserver angesprochen. In der folgenden Abbildung sind die beteiligten Komponenten dargestellt: +

image:SMTP-KIM-Adapter.svg[width=90%]

Die KIM Postfächer werden zusätzlich auf dem eigenen Mailserver eingerichtet. Auf dem SMTP-KIM-Adapter müssen die passenden Aufrufkontexte für die Kommunikation mit dem Konnektor und den zu verwendenden Smartcards (SMC-Bs und HBA) konfiguriert werden. Der Vorteil dieser Lösung ist, dass nur ein Arbeitsplatz-Identifier pro konfigurierter Smartcard vergeben werden muss, was den Konfigurationsaufwand im Konnektor und in den PS/KIS Arbeitsplätzen deutlich senken kann. Die KIM Adressen der im Mailserver konfigurierten KIM-Postfächer können durch eine VZD-Abfrage automatisch konfiguriert werden.

In Senderichtung ist der SMTP-KIM-Adapter nur ein weiterer Proxy, ähnlich wie das KIM Clientmodul. In Empfangsrichtung wird der SMTP-KIM-Adapter so konfiguriert, dass er regelmäßig (z. B. jede Minute) per POP3 die KIM Nachrichten von allen konfigurierten KIM-Adressen über das KIM Clientmodul vom KIM Mailserver abholt und per SMTP an den KKH Mailserver weiterleitet. Vom KKH Mailserver werden die Nachrichten an einen angeschlossenen AV-Scanner weitergeleitet.

* *Unterstützung mehrerer Aufrufvarianten* +
Es ist erforderlich, dass das Primärsystem bzw. das KIS die Aufrufvarianten SMTP-/POP3-Benutzername ohne Aufrufkontext und SMTP-/POP3-Benutzername mit Aufrufkontext unterstützt.
