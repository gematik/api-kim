...
1.	Der Fachdienst KOM-LE MUSS die Verarbeitung eine KOM-LE-Nachricht mit einem SMTP-Fehler ablehnen, wenn eines der folgenden Merkmale der „originator“ Header-Elemente (RFC 5322) zutrifft, zu beachten ist die unter (2) formulierte Ausnahme:
   ...
   *	Es sind mehr als genau eine Adresse im Header-Element „from“ angegeben
   ...
   *	Es sind mehr als genau eine Adresse im Header-Element „reply-to“ angegeben
   *	Es ist genau eine Adresse im Header-Element „reply-to“ angegeben und diese stimmt nicht mit der Adresse aus dem SMTP-Protokollschritt „MAIL FROM“ überein (RFC 5322 „addr-spec“)
