...
E-Mails mit einer Gesamtgröße bis zu 15 MiB werden entsprechend den Festlegungen im KIM 1.0 behandelt. Übersteigt die Größe einer E-Mail die 15 MiB Grenze, wird die gesamte Client-Mail auf den KOMLE-Attachment-Service (KAS) des Fachdiensts des Absenders abgelegt. Das Clientmodul ersetzt den Body der originalen Mail mit den Link auf die E-Mail-Daten und versendet sie als KIM-Mail. Das KIM-Clientmodul des Empfängers erkennt den Link in der KIM-Mail, lädt die E-Mail-Daten vom KAS des Absenders und entschlüsselt sie. Die damit wieder hergestellte originale Client-Mail wird dem Mail-Client des Empfängers zugestellt. Der Umgang mit großen Anhängen ist in *[gemSpec_CM_KOMLE#3.2]* spezifiziert. Die vom KAS dazu bereitgestellte Schnittstelle wird im folgenden genauer beschrieben. 

==== I_Attachment_Service

Über die Schnittstelle `I_Attachment_Service` stellt der KAS dem Clientmodul die logischen Operationen `add_Attachment()`, und `read_Attachment()` zum Hoch- und Herunterladen von verschlüsselten Anhängen zur Verfügung. Im folgenden Kapitel wird der Aufruf der Operationen beschrieben. 

image:I_KAS.png[width=45%] 

===== add_Attachment() +
Mit Hilfe der Opertion `add_Attachment()` wird ein verschlüsselter Anhang unter einem neu erzeugten Freigabe-Link auf dem KAS für einen begrenzten Empfängerkreis abgelegt. Hierfür werden in der Operation zusammen mit dem Anhang die berechtigten Empfänger, die Message-ID der dazugehörenden KIM-Nachricht und das Ablaufdatum des Anhanges übergeben.

*Beispiel einer HTTP Nachricht*
[cols="h,a"] 
|===
|URI        |\https://kas.hrst1.kim.telematik/attachments/v2.2/attachment/
|Method     |POST
|Header |
[source, bash]
----
...
[normal]#`expires` - Ablaufdatum des Anhanges# +
[normal]#`attachment` - Anhang der auf dem KAS abgelegt werden soll# 
|===
...
-----------------
curl -X 'POST' \
'https://kas.hrst1.kim.telematik/attachments/v2.2/attachment/' \
...
Body:
{
  "sharedLink":"https://kas.hrst1.kim.telematik/attachments/v2.2/attachment/b2deea19-c37f-4ef0-a95f-d4e8b5817824"
}
...
===== read_Attachment()
Die Opertion `read_Attachment()` gibt den unter einem Freigabelink verschlüsselten Anhang zurück. Beim Aufruf der Operation muss die E-Mail-Adresse des Empfängers übergeben werden. 

*Beispiel einer HTTP Nachricht*
[cols="h,a"] 
|===
|URI        |\https://kas.hersteller.kim.telematik/attachments/v2.2/attachment/{attachmentId} +
...
-----------------
curl -X 'GET' \
'https://kas.hrst1.kim.telematik/attachments/v2.2/attachment/b2deea19-c37f-4ef0-a95f-d4e8b5817824' \
...
[small]#Anhang zu oft heruntergeladen.#
|500 
|Internal Server Error
|===
...
[cols="h,a"] 
|===
|URI        |\https://account-manager.hrst1.kim.telematik/AccountMgmt/v2.2/account
|Method     |POST
|Header |
[source, bash]
----
...
[cols="h,a"] 
|===
|URI        |\https://account-manager.hrst1.kim.telematik/AccountMgmt/v2.2/account/{username} +
...
-----------------
curl -X 'DELETE' \
  'https://account-manager.hrst1.kim.telematik/AccountMgmt/v2.2/account/user@example.kim.telematik' \
  -H 'accept: */*' \
  -H 'password: password'
----------------- 
...
[cols="h,a"] 
|===
|URI        |\https://account-manager.hrst1.kim.telematik/AccountMgmt/v2.2/account/{username}/cert +
[normal]#`username` - Die E-Mail Adresse des KIM-Teilnehmers#
|Method     |POST
|Header |
[source, bash]
...
-----------------
curl -X 'POST' \
  'https://account-manager.hrst1.kim.telematik/AccountMgmt/v2.2/account/user@example.kim.telematik/cert' \
  -H 'accept: application/json' \
  -H 'password: password' \
  -H 'Content-Type: application/json' \
  -d '{
...
[cols="h,a"] 
|===
|URI        |\https://account-manager.hrst1.kim.telematik/AccountMgmt/v2.2/account/{username}/OTP +
[normal]#`username` - Die E-Mail Adresse des KIM-Teilnehmers#
|Method     |GET
|Header |
[source, bash]
...
-----------------
curl -X 'GET' \
  'https://account-manager.hrst1.kim.telematik/AccountMgmt/v2.2/account/user@example.kim.telematik/OTP' \
  -H 'accept: application/json' \
  -H 'password: password'
----------------- 
...
[cols="h,a"] 
|===
|URI        |\https://account-manager.hrst1.kim.telematik/AccountMgmt/v2.2/account/{username}/telematikID +
[normal]#`username` - Die E-Mail Adresse des KIM-Teilnehmers#
|Method     |GET
|Header |
[source, bash]
...
-----------------
curl -X 'PUT' \
  'https://account-manager.hrst1.kim.telematik/AccountMgmt/v2.2/account/user@example.kim.telematik/telematikID' \
  -H 'accept: */*' \
  -H 'password: password' \
...
[cols="h,a"] 
|===
|URI        |\https://account-manager.hrst1.kim.telematik/AccountMgmt/v2.2/account/{username}/outofoffice +
[normal]#`username` - Die E-Mail Adresse des KIM-Teilnehmers#
|Method     |GET
|Header |
[source, bash]
...
-----------------
curl -X 'GET' \
  'https://account-manager.hrst1.kim.telematik/AccountMgmt/v2.2/account/user@example.kim.telematik/outofoffice' \
  -H 'accept: application/json' \
  -H 'password: password'
----------------- 
